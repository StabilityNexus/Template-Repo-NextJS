...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <div className={cn('sticky inset-x-0 top-6 z-40 w-full transition-all duration-300', className)}>
...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <div
      ...
      className={cn(
        'relative z-[60] mx-auto hidden w-full flex-row items-center justify-between rounded-full bg-transparent px-6 py-2.5 lg:flex transition-all duration-500',
        visible 
          ? 'w-[75%] max-w-4xl bg-white/85 backdrop-blur-xl border border-black/5 shadow-lg shadow-black/[0.02]' 
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <div
      ...
      className={cn(
        'flex flex-row items-center justify-center space-x-1 text-xs font-semibold text-neutral-500 transition duration-200 lg:flex',
        className
      )}
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-neutral-400 hover:text-black transition-colors duration-200"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
...
export const MobileNav = ({ children, className, visible }: MobileNavProps) => {
  return (
    <div
      className={cn(
        'relative z-50 mx-auto flex w-full max-w-[calc(100vw-2rem)] flex-col items-center justify-between bg-transparent px-4 py-3.5 lg:hidden rounded-full transition-all duration-300',
        visible && 'bg-white/85 backdrop-blur-xl border border-black/5 shadow-md',
        className
      )}
    >
      {children}
